...
const main = async () => {
	...
	// CORRECTED: collect ALL distinct unprotected patterns across the WHOLE
	// reachable family (no early stop), not just the first one found.
	const carveOuts: Array<{ p: IGamemasterPokemon; cap: number; pattern: { A: number; D: number; S: number } }> = [];
	let multiPatternSpecies = 0;
	for (const p of candidates) {
		...
		for (const cap of CAPS) {
			...
			for (const r of reachable) {
				...
				if (!best) continue;
				if (isProtectedByBlanket(best)) continue; // this stage is fine on its own, contributes nothing
				const k = `${bucket(best.A)}-${bucket(best.D)}-${bucket(best.S)}`;
				if (!distinctPatterns.has(k)) distinctPatterns.set(k, best);
			}
			if (distinctPatterns.size > 1) multiPatternSpecies++;
			for (const pattern of distinctPatterns.values()) carveOuts.push({ p, cap, pattern });
		}
	}
	console.log(`(species, cap) pairs needing MORE THAN ONE carve-out pattern: ${multiPatternSpecies}`);
	...
	for (const { p, pattern } of carveOuts) {
		...
		if (!baseId) { console.warn(`no baseId for ${p.speciesId}`); continue; }
	...
	console.log(`Carve-out (species,cap,pattern) triples generated: ${carveOuts.length} (${seenClauses.size} unique clauses after dedup)`);
