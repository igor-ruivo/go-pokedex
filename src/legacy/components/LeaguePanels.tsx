...
const LeaguePanels = ({
	...
	const handlePanelToggle = (e: React.MouseEvent | React.KeyboardEvent) => {
		...
		(leagueStat: LeagueStat) => {
			...
			return (
				<div className={pvpStatsClassName}>
					...
					{unranked ? (
						<div className='unranked-pvp-stats pvp-entry centered-text aligned unranked unranked-panel'>
			...
			return (
				<div className='pvp-stats-column raid-gap raid full-height spaced'>
					...
					<div className={`raid-stats ${raidStat.typeRanks.length < 3 ? 'double-card' : 'triple-card'}`}>
						...
						{Array.from({
							length: Math.max(0, 2 - raidStats.typeRanks.length),
						}).map((_, i) => (
							<div key={'undefined-second-type-' + i} className='pvp-labels'>
								<RaidCard type={PokemonTypes.Normal} rank={0} dps={0} />
							</div>
						))}
					</div>
