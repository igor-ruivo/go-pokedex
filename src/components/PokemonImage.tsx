        ...
        <>
            <span className="images-container">
                <span className="main-image-container">
                    <img ref={ref} className={`${galleryToggle ? "clickable" : ""}`} onClick={galleryToggle ? handleImageClick : undefined} loading={lazy ? "lazy" : undefined} alt={pokemon.speciesName.replace("Shadow", translator(TranslatorKeys.Shadow, currentLanguage))} height={specificHeight ?? "100%"} width={specificWidth ?? "100%"} src={fetchSrc(currentImageSource)} onError={e => onError(e, currentImageSource)}/>
