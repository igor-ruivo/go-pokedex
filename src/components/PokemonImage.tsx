        ...
        <>
            <span className="images-container">
                <span className="main-image-container">
                    ...
                    {false && shiny && <img alt="shiny" className='img-adorner image shiny-overlay invert-dark-mode' loading={lazy ? "lazy" : undefined} src={`/vectors/shiny.svg`} width="100%" height="100%"/>}
            ...
            {withName && <div style={{lineHeight: 1.2}}><span className="pkm-img-name ellipsed block-column minimal-padding-bottom" style={specificNameContainerWidth ? {width: specificNameContainerWidth} : {}}>{shortName(pokemon.speciesName) + (forceShadowAdorner ? " (S)" : "")}</span></div>}
        </>
    )
});
