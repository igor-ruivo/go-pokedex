...
const DeleteTrash = () => {
	...
	const enumValues: Array<PokemonTypes> = useMemo(
		() =>
	...
	const isBadForEverythingIfItHasHighAttack = useCallback(
		...
		[
			...
			trashMaster,
			isBadRank,
			needsLessThanFiveAttack,
			trashGreat,
			trashUltra,
		]
	);

	const isBadForEverything = useCallback(
		(p: IGamemasterPokemon) => {
			if (!fetchCompleted || !pvpFetchCompleted) {
				return false;
			}
	...
	const computeStr = useCallback(() => {
		...
		gamemasterPokemon,
		isBadForEverything,
		isBadForEverythingIfItHasHighAttack,
		cp,
		currentGameLanguage,
	]);
	...
	// Handler for copying textarea content to clipboard, with proper typing
									...
									<div className='flex-column normal-margin-top'>
										...
										>
											...
											<u
												...
												onKeyDown={handleReadMoreToggle}
												id='readMoreLink'
												tabIndex={0}
												role='button'
											...
											<div className='row-flex'>
												<div>
													...
													<select value={cp} onChange={(e) => setCP(+e.target.value)} className='select-level'>
														...
														<option key={2500} value={2500}>
															{2500}
														</option>
														<option key={3000} value={3000}>
															{3000}
														</option>
														<option key={3500} value={3500}>
															{3500}
														</option>
											...
											<div className='row-flex'>
												...
												<div>
													<img
														height={20}
														width={20}
														className='padded-img raid-img-with-contrast'
														alt='Raids'
										...
										>
											{translator(TranslatorKeys.Compute, currentLanguage)}!
										</button>
										<textarea
											ref={targetRef}
