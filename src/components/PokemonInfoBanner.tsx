...
const PokemonInfoBanner = ({pokemon, ivPercents, attack, setAttack, defense, setDefense, hp, setHP, league, handleSetLeague, level}: IPokemonInfoBanner) => {
    ...
    const ranksComputation = useMemo(() => {
        ...
        return ranksDic;
    }, [fetchCompleted, raidDPSFetchCompleted, movesFetchCompleted, allRelevantChargedMoveTypes, raidDPS, bestReachableRaidVersion, attack, level, moves, gamemasterPokemon, pokemon]);

    const resourcesNotReady = useMemo(() => !raidDPSFetchCompleted || !bestReachableRaidVersion || !fetchCompleted || !pokemon || !pvpFetchCompleted || !movesFetchCompleted || !gamemasterPokemon || !moves || Object.keys(moves).length === 0 || rankLists.length === 0 || Object.keys(ivPercents).length === 0
    , [bestReachableRaidVersion, fetchCompleted, gamemasterPokemon, ivPercents, moves, movesFetchCompleted, pokemon, pvpFetchCompleted, raidDPSFetchCompleted, rankLists]);
    ...
    }, [fetchCompleted, allReachableRaidPokemon, movesFetchCompleted, gamemasterPokemon, pokemon, raidDPS, raidDPSFetchCompleted, setBestReachableRaidVersion, currentBestReachableRaidLeagueIndex]);
                                            ...
                                            .map(t => (
                                                <div key = {t}>
                                                    <strong className={`move-detail ${superEffective.includes(t) ? "special-item" : ""} relative soft family-padding item`}>
                                                        ...
                                                        <div className="img-padding"><img className="with-img-dropShadow" height={20} width={20} alt="type" src={`${process.env.PUBLIC_URL}/images/types/${t.toLocaleLowerCase()}.png`}/></div>
                                                    </strong>
                                                </div>
                                            ...
                                            .map(t => (
                                                <div key = {t}>
                                                    <strong className={`move-detail ${tripleResistance.includes(t)  ? "triple-item" : superResistance.includes(t) ? "special-item" : ""} relative soft family-padding item`}>
                                                        ...
                                                        <div className="img-padding"><img height={20} className="with-img-dropShadow" width={20} alt="type" src={`${process.env.PUBLIC_URL}/images/types/${t.toLocaleLowerCase()}.png`}/></div>
                                                    </strong>
                                                </div>
                                            )) : <sub className="weighted-font">{translator(TranslatorKeys.Nothing, currentLanguage)}</sub>}
                                        </div>
