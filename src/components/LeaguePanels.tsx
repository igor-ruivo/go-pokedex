                            ...
                            <div className="pvp-entry clickable" onClick={() => setToggled(p => !p)}>
                                <div className="pvp-entry-content potential">
                                    <>
                                        ...
                                        level > leagueStat.pokemonLevel ? <span className="higher-level weighted-font">{translator(TranslatorKeys.LevelExceeded, currentLanguage)}</span> : level === leagueStat.pokemonLevel ? <span className="buffed weighted-font with-brightness">{translator(TranslatorKeys.Reached, currentLanguage)}</span> : <div className="needed-resources">
                                            <img src={`${process.env.PUBLIC_URL}/images/stardust.png`} alt="stardust" height={16} width={16}/><div className="weighted-font cp-container">{neededResources.stardust > 1000 ? Math.round(Math.round(neededResources.stardust / 1000) * 10) / 10 + "K" : neededResources.stardust}</div>
                                            {neededResources.candies !== 0 && <><img src={`${process.env.PUBLIC_URL}/images/candy.png`} alt="candy" height={16} width={16}/><div className="weighted-font cp-container">{neededResources.candies}</div></>}
                                            {neededResources.candiesXL !== 0 && <><img src={`${process.env.PUBLIC_URL}/images/xl-candy.png`} alt="xl-candy" height={16} width={16}/><div className="weighted-font cp-container">{neededResources.candiesXL}</div></>}
                                        </div>}
                                    </>
                                </div>
                            </div>
