...
const LeagueRanks = ({
	...
	const computeRankChange = useCallback(
		(speciesId: string, leagueTitle: string) => {
	...
	const rankChangeClassName = useCallback(
		(speciesId: string, leagueTitle: string) =>
			!pvpFetchCompleted || !rankLists[getLeagueType(leagueTitle)][speciesId]
				? ''
				: rankLists[getLeagueType(leagueTitle)][speciesId].rankChange === 0
					? 'neutral'
					: rankLists[getLeagueType(leagueTitle)][speciesId].rankChange < 0
						? 'nerfed'
						: 'buffed',
					...
					extraIcons={[
						{
							...
							image: (
								<PokemonImage
									pokemon={leagueStat.bestReachablePokemon}
									withName={false}
									specificWidth={28}
									specificHeight={28}
								/>
						...
						<React.Fragment key={leagueStat.leagueTitle}>
							...
							{leagueStat.type && (
								<img
									...
									src={`/images/types/${leagueStat.type}.png`}
									height={20}
									width={20}
								/>
							)}
	...
	return (
		<div className='default-padding pvp-leagues-2 moves-list slim-list'>
